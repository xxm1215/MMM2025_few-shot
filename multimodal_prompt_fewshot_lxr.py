...
def mini_batching(inputs):
    ...
    sim_all = []
    proj = Proj_layers()
    cat=CrossAttention(512,512,512,512,8)#cross-attetion
    sim = torch.nn.CosineSimilarity(dim=1, eps=1e-6)
    sig = torch.nn.Sigmoid()
    for item in inputs['guid']:
        for sample in all_data:
    ...
    for item in inputs['guid']:
        for sample in all_data:
            if item == sample['id']:
                ...
                i_emb = model.encode_image(i_input)#[1,512]
                t_emb = model.encode_text(t_input)*emo_score#[1,512]
...
def start_training(model, train_dataloader, val_dataloader,
    ...
    for epoch in range(epoch):
        ...
        for inputs in tqdm(train_dataloader):
            ...
            out = alpha * mini_batch + out+(1-alpha)*inter_batch
    ...
    if str(args.data) == "goss":
        if t_or_f(args.full):
            directory = "./results_excel_8heads/goss/full/"
        else:
            directory = "./results_excel_8heads/goss/few/"
    else:
        if t_or_f(args.full):
            directory = "./results_excel/poli/full/"
        else:
            directory = "./results_excel/poli/few/"
...
if __name__ == "__main__":
    ...
    goss_path1 = "/root/autodl-tmp/ccl2024/datasets/fakenewsnet/goss_img_all/*.jpg"
    goss_path2 = "/root/autodl-tmp/ccl2024/datasets/fakenewsnet/goss_img_all/"
    goss_path3 = '/root/autodl-tmp/ccl2024/datasets/fakenewsnet/normalized_data_emoscore_goss.csv'

    poli_path1 = "/root/autodl-tmp/ccl2024/datasets/fakenewsnet/poli_img_all/*.jpg"
    poli_path2 = "/root/autodl-tmp/ccl2024/datasets/fakenewsnet/poli_img_all/"
    poli_path3 = '/root/autodl-tmp/ccl2024/datasets/fakenewsnet/normalized_data_emoscore_poli.csv'
    if str(args.data) == "goss":
